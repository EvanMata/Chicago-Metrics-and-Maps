...
'''
This code details the mechanisms to clean and interact with the crime data.
In addition to the code written here, there were nnumerous generally simple 
SQL commands, mostly used for diagnosis of problems. They included many cases
such as SELECT IUCR FROM crime WHERE Latitude > 46.784. Two distinct uses will
be noted later as they were used instead of functions provided here.
'''
...
def clean_iucrs2():
	'''
	Note this function ended up ruining the DB and so was not used. 
	This was because apparently	the IUCR_codes in the IUCR table both do and
	do not exclude leading 0's.
	This was finally understood with the following SQL statement: 
	SELECT IUCR_code FROM IUCR where IUCR_code = 0110;
	The output of this is 110. This happends for all IUCR codes with leading
	zeros. I thought this needed cleaning, but apparently this still matches
	with the IUCR codes in the crime db, which appear in the 0110 format.
	'''
	'''
...
def test_empty_lat_lons():
	#Ensure I have no Null latitutdes.
	connection = sqlite3.connect(DATABASE_NAME)
	c = connection.cursor()
	command1 = '''
...
def correct_iucr2(iucr):
	...
	(where d = digit).
	'''
	#Note this was the function ocrresponding to the db-breaking 0110-110 case
...
def find_crimescore(input_long, input_lat):
	...
	Inputs: 
			input_long - A float representing the longitude you're searching.
			input_lat - A float representing the latitude you're searching.
	Returns:
			cs - A float representing your crimescore.
	'''
	start = time.time()
...
def csv_to_sql_db(csv_file, table_name, column_names):
	'''
	This is the other function here not used - though this was simply never
	working. Originally there was a csv_file that contained the first 200k
	lines of our data that I was going to make into a db. 
	It is much impler to put a csv into a db by simply opening sql and typing
	.mode csv
	.import <csv_file_dir> <table_name>
	Note that running these through python & the execute command was tried,
	but does not work.
	'''
	'''
...
def maxs_and_mins():
	...
	if we use them, almost all other datapoints show up in Chicago. 
	So we set our own bounds, from the original testing db 
	(significantly more reasonable). This function was used often in testing.
	'''
...
def rate_of_change_heatmap(year1, year2):
	...
	Inputs:
			year1 - A float or string of the year for the first heatmap.
			year2 - A float or string of the year for the second heatmap.
	'''
	#Note that due to the saving format of our prior information this works.
...
def heat_map(year):
	...
	Input:
		year - A string representing the year to make a heatmap for.
	'''
	start = time.time()
	connection = sqlite3.connect(DATABASE_NAME)
	c = connection.cursor()
...
def to_geo_json(d, id1, x_min, x_max, y_min, y_max):
	'''
	We hoped to use the nicer heatmapping visuals of other project members
	for already-computed data saved from my heatmap functions. They required
	a geojason data format, so this function creates that format. 
	Inputs: 
			d - the dictionary your information is added to.
			id1 - A string id providing your information's id. This was used
			to fit with the format of the other members code.
			x_min - A float or string numeric value.
			x_max - A float or string numeric value.
			y_min - A float or string numeric value.
			y_max - A float or string numeric value.
	'''
...
def post_processing(Mi_long, Ma_long, Mi_lat, Ma_lat, previous_info):
	'''
	As noted, we were attempting to take our previously generated data and 
	construct a nice form heatmap for it. The previously saved files only
	saved the heatmap values to a file, not the coordinates they used. 
	Thus we need to reconstruct that information, and compile our geo json
	from it. This information was then saved. A pandas dataframe was also
	constructed to fit the format of the partners code.
	Note that this function outputs what appears correct, but does not work
	with the code the partner write. Therefore this was not used in the end.
	Inputs:
			Mi_long - A float, our minimum longitude from the original data.
			Ma_long - A float, our maximum longitude.
			Mi_lat - A float, our minimum latitude.
			Ma_lat - A float, our maximum latitude.
			previous_info - The name of the csv file that stored our data.
	'''
...
def Hyde_park_heatmap():
	...
	and no year specification. This can in theory be generalized to input 
	lats, longs and years if desired. 
	'''
	start = time.time()
	connection = sqlite3.connect(DATABASE_NAME)
	c = connection.cursor()
